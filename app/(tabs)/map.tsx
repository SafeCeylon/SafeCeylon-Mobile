...
const MapPage: React.FC = () => {
  ...
  const submitDisasterReport = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.1.44:8080/api/users/report-disaster', disasterData);
  ...
  useEffect(() => {
    const fetchMapData = async () => {
      try {
        const response = await axios.get('http://192.168.1.44:8080/api/users/map');
        if (response.status === 200) {
