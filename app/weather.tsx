...
const MarineWeatherScreen: React.FC = () => {
    ...
    const getDayName = (offset: number) => {
        ...
        if (offset === 0) {
          ...
          const month = futureDate.toLocaleString('default', { month: 'short' }).toUpperCase();
          return `${day} ${month}`;
        }
      };
    ...
    const dummyWeatherData = [
        "Clear Sky", "Partly Cloudy", "Mostly Cloudy", "Rain Showers", "Thunderstorms", "Sunny", "Overcast"
    ];
    ...
    const generateDateRanges = () => {
      ...
      const timeRange = (date: Date) => {
        ...
        return `${date.toLocaleDateString('en-US', dateOptions)} 8.30AM - ${tomorrow.toLocaleDateString('en-US', dateOptions)} 8.30AM`;
      };
  ...
  const rainfallImages = [
    rainfallImage0, rainfallImage1, rainfallImage2,
    rainfallImage3, rainfallImage4, rainfallImage5,
    rainfallImage6, rainfallImage7, rainfallImage8,
  ];

  const temperatureImages = [
    temperatureImage0, temperatureImage1, temperatureImage2,
    temperatureImage3, temperatureImage4, temperatureImage5,
    temperatureImage6, temperatureImage7, temperatureImage8,
  ];
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        {selectedWeather === "Weather" && (
            ...
            <View style={styles.weatherContainer}>
              ...
              <Text style={styles.weatherInfo}>{dummyTemperatureData[hour / 3]}°C | {dummyWeatherData[hour / 3]}</Text>
              <Text style={styles.weatherDetails}>Today | Saturday | {hour}:00 PM</Text>
              <Text style={styles.weatherDetails}>Humidity: 55% | Wind: 23 km/h</Text>
            </View>

            <View style={styles.adjustContainer}>
              <TouchableOpacity onPress={() => handleHourChange(false)} style={styles.button}>
                <Text style={styles.buttonText}>-</Text>
              </TouchableOpacity>
              <Text style={styles.adjustLabel}>Adjust Hour: {hour}:00</Text>
              <TouchableOpacity onPress={() => handleHourChange(true)} style={styles.button}>
                <Text style={styles.buttonText}>+</Text>
              </TouchableOpacity>
            </View>

            <View style={styles.adjustContainer}>
              <TouchableOpacity onPress={() => handleDayChange(false)} style={styles.button}>
                <Text style={styles.buttonText}>-</Text>
              </TouchableOpacity>
              <Text style={styles.adjustLabel}>Adjust Day: {getDayName(dayOffset)}</Text>
              <TouchableOpacity onPress={() => handleDayChange(true)} style={styles.button}>
                <Text style={styles.buttonText}>+</Text>
              </TouchableOpacity>
            </View>

            <View style={styles.dayTempContainer}>
              <Text style={styles.dayTemp}>{getDayName(dayOffset)}: Max {dummyTemperatureData[dayOffset]}°C | Min {dummyTemperatureData[(dayOffset + 1) % 9]}°C</Text>
            </View>
          </View>
        )}
        ...
        {selectedWeather === "Marine Weather" && (
          <View style={styles.card}>
            ...
            <Text style={styles.cardDescription}>
              Wind will be westerly to south-westerly and wind speed will be (30-40) kmph. Wind speed can increase up to 50 kmph at times in the sea areas off the coasts extending from Kankasanthurai to Puttalam via Mannar and from Hambantota to Pottuvil.
            </Text>

            <Text style={styles.cardTitle}>State of Sea:</Text>
            <Text style={styles.cardDescription}>
              The sea areas off the coasts extending from Trincomalee to Kankasanthurai via Mullaitivu and Puttalam to Hambantota via Colombo and Galle can be fairly rough at times.
            </Text>
          </View>
        )}
        ...
        {selectedWeather === "City Forecast" && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Issued at 04.00 p.m. on 26 July 2024</Text>
            <Text style={styles.cardDescription}>
              Showers will occur at times in Sabaragamuwa province and in Kandy and Nuwara-Eliya districts. Several spells of showers will occur in Western and North-western provinces and in Galle and Matara districts.
            </Text>
            <Text style={styles.cardDescription}>
              Strong winds of about (50-55) kmph can be expected at times over Western slopes of the central hills and Northern, North-central and North-western provinces and in Trincomalee, Monaragala and Hambantota districts. Temporary strong winds (about 40-50) kmph can be expected during thundershowers elsewhere of the island.
            </Text>
          </View>
        )}
        ...
        {selectedWeather === "9 Day Forecast" && (
          <>
            ...
            <View style={styles.card}>
              <Text style={styles.imageType}>{currentImageIndex === 0 ? "Rainfall" : "Temperature"}</Text>
              <Image source={forecastImages[currentImageIndex][selectedDayIndex]} style={styles.forecastImage} />
