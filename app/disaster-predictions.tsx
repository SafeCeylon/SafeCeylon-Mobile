...
const DisasterPrediction: React.FC = () => {
    ...
    const districts = [
        "Colombo", "Kandy", "Galle", "Jaffna", "Gampaha", "Matara", "Kurunegala", "Anuradhapura", "Badulla",
        "Batticaloa", "Hambantota", "Kilinochchi", "Mannar", "Matale", "Monaragala", "Mullaitivu", "NuwaraEliya",
        "Polonnaruwa", "Puttalam", "Ratnapura", "Trincomalee", "Vavuniya", "Kegalle", "Kalutara", "Ampara"
    ];
    ...
    const renderWarningLevels = () => {
        if (selectedDisaster === "Landslide" && gnDivisionsDataLS[selectedDistrict]) {
            return (
                <View style={styles.warningContainer}>
                    {disasterData.Landslide.warnings.map((warning, index) => (
                        <View key={index} style={[styles.warningBox, { backgroundColor: warning.color }]}>
                            <Text style={styles.warningText}>Warning Level: {warning.level}</Text>
                            {gnDivisionsDataLS[selectedDistrict][warning.level] && (
                                <View style={styles.gnDivisionContainer}>
                                    {gnDivisionsDataLS[selectedDistrict][warning.level].map((gnDivision, idx) => (
                                        <Text key={idx} style={styles.gnDivisionText}>{gnDivision}</Text>
                                    ))}
                                </View>
                            )}
                        </View>
        ...
        } else if (selectedDisaster === "Hurricane"  && gnDivisionsDataHR[selectedDistrict]) {
            return (
                <View style={styles.warningContainer}>
                    {disasterData.Landslide.warnings.map((warning, index) => (
                        <View key={index} style={[styles.warningBox, { backgroundColor: warning.color }]}>
                            ...
                            {gnDivisionsDataHR[selectedDistrict][warning.level] && (
                                <View style={styles.gnDivisionContainer}>
                                    {gnDivisionsDataHR[selectedDistrict][warning.level].map((gnDivision, idx) => (
                                        <Text key={idx} style={styles.gnDivisionText}>{gnDivision}</Text>
                                    ))}
                                </View>
                            )}
        ...
        } else if (selectedDisaster === "Floods"  && gnDivisionsDataFL[selectedDistrict]) {
            return (
                <View style={styles.warningContainer}>
                    {disasterData.Landslide.warnings.map((warning, index) => (
                        <View key={index} style={[styles.warningBox, { backgroundColor: warning.color }]}>
                            ...
                            {gnDivisionsDataFL[selectedDistrict][warning.level] && (
                                <View style={styles.gnDivisionContainer}>
                                    {gnDivisionsDataFL[selectedDistrict][warning.level].map((gnDivision, idx) => (
                                        <Text key={idx} style={styles.gnDivisionText}>{gnDivision}</Text>
                                    ))}
                                </View>
                            )}
